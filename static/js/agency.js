...
async function loadAgencyCategories() {
    try {
        ...
        const protectedCats = ['TAB', 'INJ', 'CAP', 'SYP', 'CRM', 'GEL', 'DROP', 'POW', 'LOT', 'SPRAY', 'OINT', 'SURGICAL', 'OTHER', 'TABLET', 'INJECTION'];
        document.getElementById('agCatBody').innerHTML = agencyCategoriesData.map(c => {
            const isProtected = protectedCats.includes((c.name || '').toUpperCase().trim());
            const deleteBtn = isProtected 
                ? `<span class="badge" style="background:var(--border); color:var(--text-muted); font-size:0.75rem;">System</span>` 
                : `<button class="btn btn-outline btn-sm" style="color:var(--danger); border-color:var(--danger);" onclick="deleteAgencyCat(${c.id})" title="Delete Custom Category"><i class="fas fa-trash"></i> Delete</button>`;
            return `
            <tr>
                <td><strong>${c.name}</strong> ${!isProtected ? '<span class="badge" style="background:rgba(59,130,246,0.15); color:var(--accent); font-size:0.7rem; margin-left:6px;">Custom</span>' : ''}</td>
                <td>
...
window.gmEditGenericName = function(genericName) {
    ...
    const otherInput = document.getElementById('gmEditGenericCategoryOther');
    ...
    if (catSelect) {
        ...
        if (!isKnown && genericName.includes('(') && genericName.endsWith(')')) {
            const match = genericName.match(/\(([^)]+)\)$/);
            if (match && match[1]) {
                category = match[1];
                baseName = genericName.substring(0, genericName.lastIndexOf('(')).trim();
            }
        }
        if (isKnown || !category) {
            catSelect.value = category;
            if (typeof toggleCategoryOtherInput === 'function') toggleCategoryOtherInput(catSelect, 'gmEditGenericCategoryOther');
        } else {
            catSelect.value = 'Other';
            if (typeof toggleCategoryOtherInput === 'function') toggleCategoryOtherInput(catSelect, 'gmEditGenericCategoryOther');
            if (otherInput) otherInput.value = category;
        }
    }
...
window.gmSaveGenericName = async function() {
    ...
    if (catSelect && catSelect.value) {
        ...
        if (categoryValue === 'Other') {
            const otherInput = document.getElementById('gmEditGenericCategoryOther');
            categoryValue = (otherInput ? otherInput.value : '').trim();
            if (!categoryValue) {
                toast('Please enter custom category name', 'error');
                return;
            }
            try {
                await api('/api/agency/categories/add', { method: 'POST', body: { name: categoryValue } });
            } catch (e) {}
        }
...
window.handleGenericImportPdf = function(input) {
    ...
    reader.onload = async function(e) {
        try {
            ...
            } else if (typeof window.pdfjsLib === 'undefined' && typeof window['pdfjs-dist/build/pdf'] === 'undefined') {
            ...
            const pdfjsLib = window.pdfjsLib || window['pdfjs-dist/build/pdf'] || (typeof pdfjsLib !== 'undefined' ? pdfjsLib : null);
            if (!pdfjsLib) {
    ...
    try {
        ...
        if (response.success) {
            if (typeof Swal !== 'undefined' && Swal.fire) {
                ...
                toast(response.message || 'Mappings imported successfully', 'success');
            }
            loadGenericMedicines();
        } else {
            if (typeof Swal !== 'undefined' && Swal.fire) {
                ...
                toast(response.error || 'Failed to import mappings', 'error');
            }
        }
    } catch (e) {
        if (typeof Swal !== 'undefined' && Swal.fire) {
            ...
            toast(e.message || 'Failed to import mappings', 'error');
        }
    }
}
// ═══════════════════════════════════════════
...
window.toggleCategoryOtherInput = function(selectEl, otherInputId) {
...
window.openAddGenericItemModal = function() {
    ...
    if (catEl) {
        catEl.value = 'TAB';
        toggleCategoryOtherInput(catEl, 'gmAddCategoryOther');
    }
...
window.gmSaveNewItem = async function() {
    ...
    const catEl = document.getElementById('gmAddCategory');
    let category = (catEl ? catEl.value : 'TAB').trim();
    if (category === 'Other') {
        const otherEl = document.getElementById('gmAddCategoryOther');
        const customCat = (otherEl ? otherEl.value : '').trim();
        if (!customCat) {
            toast('Please enter custom category name.', 'error');
            return;
        }
        category = customCat;
        // Optionally store custom category in DB table agency_categories
        try {
            await api('/api/agency/categories/add', { method: 'POST', body: { name: category } });
        } catch (e) {}
    }
    try {
...
window.gmFilterList = gmFilterList;
window.renderGmList = renderGmList;
window.gmViewBrands = gmViewBrands;
window.gmBackToList = gmBackToList;
window.gmDeleteGeneric = gmDeleteGeneric;
