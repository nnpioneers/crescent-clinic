            ...
            invSearchTimeout = setTimeout(async () => {
                ...
                try {
                    ...
                    if (isTablet && i.tablets_per_strip > 0) {
                        const isNeg = i.stock < 0;
                        const absStock = Math.abs(i.stock);
                        const strips = Math.floor(absStock / i.tablets_per_strip);
                        const sign = isNeg ? '-' : '';
                        displayStock = `${sign}${strips} Strips <br><small style="color:var(--text-muted)">(${i.stock} Tablets)</small>`;
                    }
        ...
        async function populateInvCat() {
            ...
            if (Array.isArray(agencyCats)) {
                agencyCats.forEach(c => {
                    if (c.name && !c.name.startsWith('PH3_CAT_') && !standardCategories.includes(c.name)) {
            ...
            if (typeof inventoryData !== 'undefined' && Array.isArray(inventoryData)) {
                const existingCats = [...new Set(inventoryData.map(i => i.category))].filter(c => c && !c.startsWith('PH3_CAT_') && !standardCategories.includes(c) && !agencyCats.some(ac => ac.name === c));
        ...
        async function editMedModal(item) {
            ...
            let itemCat = item.category || 'TAB';
            let itemCatLower = itemCat.toLowerCase();
            if (itemCatLower === 'tablet' || itemCatLower === 'tablets' || itemCatLower === 'tab') {
                itemCat = 'TAB';
            } else if (itemCatLower === 'injection' || itemCatLower === 'injections' || itemCatLower === 'inj') {
                itemCat = 'INJ';
            }

            let isStandardOrKnown = false;
            for (let i = 0; i < sel.options.length; i++) {
                if (sel.options[i].value.toLowerCase() === itemCat.toLowerCase() && itemCat !== 'Other' && itemCat !== '') {
                    sel.selectedIndex = i;
        ...
        async function saveInventory() {
            ...
            let catLower = (catValue || '').toLowerCase();
            if (catLower === 'tablet' || catLower === 'tablets' || catLower === 'tab') {
                catValue = 'TAB';
            } else if (catLower === 'injection' || catLower === 'injections' || catLower === 'inj') {
                catValue = 'INJ';
            }
            catLower = catValue.toLowerCase();
            const isTablet = catLower === 'tablet' || catLower === 'tab' || catLower === 'tablets';
            const payload = {
