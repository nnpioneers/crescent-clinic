...
function ensure_synthesized_inventory($conn, $name) {
    ...
    if (!$stmt->fetch()) {
        ...
        $conn->prepare("INSERT INTO inventory (name, generic_name, brand_name, stock, batch_number, category) VALUES (?, ?, '(Unmapped Brand)', 0, ?, 'Tablet')")
             ->execute([$name, $gen_name, $ph_batch]);
    }
}
