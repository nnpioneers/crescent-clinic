...
function ensure_migrations($conn)
{
    try {
        // Ensure sessions table exists for DatabaseSessionHandler
        ...
        $col_migrations = [
            ...
            'supplier_id'  => "ALTER TABLE inventory ADD COLUMN supplier_id INT DEFAULT NULL"
        ];
        ...
        // Ensure mfg_date and expiry_date are VARCHAR(100) so MM-YY dates persist without turning into 0000-00-00
        try { $conn->exec("ALTER TABLE inventory MODIFY COLUMN mfg_date VARCHAR(100) DEFAULT NULL"); } catch (Exception $e) {}
        try { $conn->exec("ALTER TABLE inventory MODIFY COLUMN expiry_date VARCHAR(100) DEFAULT NULL"); } catch (Exception $e) {}
        try { $conn->exec("ALTER TABLE generic_mappings MODIFY COLUMN expiry_date VARCHAR(100) DEFAULT NULL"); } catch (Exception $e) {}

        // Columns for agency_items
        $existing_agency_cols = [];
        try {
            $qa = $conn->query("SHOW COLUMNS FROM agency_items");
            if ($qa) {
                $all_agency_rows = $qa->fetchAll(PDO::FETCH_ASSOC);
                foreach ($all_agency_rows as $row) {
                    $existing_agency_cols[] = strtolower($row['Field']);
                }
                $qa->closeCursor();
            }
        } catch (Exception $e) {}

        $agency_col_migrations = [
            'row_location' => "ALTER TABLE agency_items ADD COLUMN row_location VARCHAR(100) DEFAULT NULL",
            'col_location' => "ALTER TABLE agency_items ADD COLUMN col_location VARCHAR(100) DEFAULT NULL"
        ];

        foreach ($agency_col_migrations as $col_name => $sql) {
            if (empty($existing_agency_cols) || !in_array(strtolower($col_name), $existing_agency_cols)) {
                try { $conn->exec($sql); } catch (Exception $e) {}
            }
        }
        ...
        try {
            $conn->exec("
                CREATE TABLE IF NOT EXISTS generic_mappings (
                    ...
                    mfg_date VARCHAR(100) DEFAULT NULL,
        ...
        // Columns for users
        try {
            $stmt_u = $conn->query("SHOW COLUMNS FROM users");
            $cols_u = $stmt_u ? $stmt_u->fetchAll(PDO::FETCH_COLUMN, 0) : [];
            if (!in_array('details', $cols_u)) $conn->exec("ALTER TABLE users ADD COLUMN details TEXT");
            if (!in_array('photo_path', $cols_u)) $conn->exec("ALTER TABLE users ADD COLUMN photo_path VARCHAR(255)");
            if (!in_array('specialization', $cols_u)) $conn->exec("ALTER TABLE users ADD COLUMN specialization VARCHAR(255)");
            if (!in_array('admin_security_password', $cols_u)) $conn->exec("ALTER TABLE users ADD COLUMN admin_security_password VARCHAR(255) DEFAULT '123'");
            if (!in_array('token_prefix', $cols_u)) $conn->exec("ALTER TABLE users ADD COLUMN token_prefix VARCHAR(50)");
            if (!in_array('is_active', $cols_u)) $conn->exec("ALTER TABLE users ADD COLUMN is_active TINYINT DEFAULT 1");
            if (!in_array('doctor_registration_number', $cols_u)) $conn->exec("ALTER TABLE users ADD COLUMN doctor_registration_number VARCHAR(255)");
        } catch (Exception $e) {}

        // Add High-Performance Database Indexes
        $performance_indexes = [
            "ALTER TABLE patients ADD INDEX idx_patients_created (created_at)",
            "ALTER TABLE patients ADD INDEX idx_patients_phone (phone)",
            "ALTER TABLE patients ADD INDEX idx_patients_doc_status (doctor_id, status)",
            "ALTER TABLE prescriptions ADD INDEX idx_presc_patient (patient_id, created_at)",
            "ALTER TABLE prescriptions ADD INDEX idx_presc_created (created_at)",
            "ALTER TABLE direct_sales ADD INDEX idx_ds_created (created_at)",
            "ALTER TABLE direct_sales ADD INDEX idx_ds_mobile (mobile_number)",
            "ALTER TABLE inventory ADD INDEX idx_inv_name (name)",
            "ALTER TABLE inventory ADD INDEX idx_inv_generic (generic_name)",
            "ALTER TABLE inventory ADD INDEX idx_inv_cat (category)",
            "ALTER TABLE sessions ADD INDEX idx_sess_exp (expires_at)"
        ];
        foreach ($performance_indexes as $idx_sql) {
            try { $conn->exec($idx_sql); } catch (Exception $e) {}
        }
    } catch (Exception $e) {}
}
