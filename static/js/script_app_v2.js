...
// Global CSRF fetch interceptor: ensures EVERY state-changing HTTP request (POST, PUT, PATCH, DELETE)
// carries the active session CSRF token in both X-CSRF-Token header and request body.
(function() {
        ...
        if (method !== 'GET') {
            ...
            if (token) {
                ...
                if (typeof Headers !== 'undefined' && headers instanceof Headers) {
                    if (!headers.has('X-CSRF-Token')) headers.append('X-CSRF-Token', token);
                } else if (Array.isArray(headers)) {
                    if (!headers.some(h => String(h[0]).toLowerCase() === 'x-csrf-token')) {
...
(function () {
    ...
    document.addEventListener("DOMContentLoaded", async () => {
        ...
        // Reception and Doctor portals do not use UPI accounts; only Pharmacy & Management use them.
        if (PAGE_ROLE !== 'receptionist' && PAGE_ROLE !== 'doctor' && PAGE_ROLE !== 'login') {
            try {
    ...
    let _liveSyncDebounceTimer = null;
    window.hospitalSyncChannel.onmessage = (event) => {
        if (event.data && event.data.type === 'DATA_UPDATED') {
            clearTimeout(_liveSyncDebounceTimer);
            _liveSyncDebounceTimer = setTimeout(() => {
    ...
    window.reloadCurrentSection = function () {
        window.receptionPatientsDirty = true;
    ...
    window.api = async function (url, opts = {}) {
        ...
        if (reqMethod === 'GET') {
            // Avoid unnecessary cache-busting for safe static master data (e.g., /api/doctors) unless explicitly forced
            const isSafeToCache = opts.cache === true || (opts.cache !== false && (url.includes('/api/doctors') || url.includes('/api/upi_accounts')));
            if (!isSafeToCache) {
    ...
    window.showSection = function (name) {
        ...
        const wasAlreadyActive = sec && sec.classList.contains('active');
        ...
        // If the user clicked the tab that is already active, do not re-fetch
        if (wasAlreadyActive) {
            window.scrollTo(0, 0);
            return;
        }
        ...
        if (name === 'patients' || name === 'queue' || name === 'prescriptions') {
            const hasFreshData = PAGE_ROLE === 'receptionist' &&
                Array.isArray(window.currentPatients) &&
                window.lastPatientsLoadedAt &&
                !window.receptionPatientsDirty &&
                (Date.now() - window.lastPatientsLoadedAt < 30000);
    ...
    if (PAGE_ROLE === 'receptionist') {
        ...
        window.registerPatient = async function (e) {
            ...
            const patDoctorEl = $('#patDoctor');
            const nameVal = patNameEl ? patNameEl.value.trim() : '';
            const doctorIdVal = patDoctorEl ? patDoctorEl.value : '';

            if (!nameVal) {
                toast('Please enter patient name', 'error');
                if (patNameEl) patNameEl.focus();
                return;
            }

            if (!doctorIdVal) {
                toast('Please select a doctor', 'error');
                if (patDoctorEl) patDoctorEl.focus();
                return;
            }

            const rawAge = $('#patAge')?.value;
            const parsedAge = (rawAge !== undefined && rawAge !== '') ? parseInt(rawAge, 10) : 0;

            const data = {
                name: nameVal,
                age: isNaN(parsedAge) ? 0 : parsedAge,
                gender: $('#patGender')?.value || 'Other',
                ...
                spo2: ($('#patSpO2') && $('#patSpO2').value !== '') ? parseInt($('#patSpO2').value, 10) : null,
            ...
            try {
                ...
                if (resRaw.ok && res.success) {
                    toast(res.message || 'Patient registered successfully!');
                    // Show token
                    ...
                    $('#registerForm').reset();
                    clearSelectedToken();
                    const banner = $('#receptionLastVisitBanner');
                    if (banner) banner.style.display = 'none';
                    const quickBox = $('#fetchQuickLastVisit');
                    if (quickBox) quickBox.style.display = 'none';
                    window.lastHandledPhone = null;
                    loadPatients();
                } else {
        ...
        window.autoFetchPatient = function (val) {
            const query = String(val || '').trim();
            if (query.length >= 10 || (query.toUpperCase().startsWith('CCS') && query.length >= 3)) {
        ...
        window.fetchPatient = async function () {
            ...
            try {
                ...
                if (res.found) {
                            ...
                            resultsList.innerHTML = res.patients.map((p, i) => `
                                <div class="stat-card" style="padding:12px; cursor:pointer; flex-direction:row; align-items:center; justify-content:space-between; margin:0; border-color:var(--border);" onclick="applyPatientDataByIndex(${i})">
                                    <div style="display:flex; flex-direction:column; gap:2px;">
                                        <div style="font-weight:600; font-family:var(--font-heading);">${p.name} <span style="font-size:0.75rem; color:var(--text-muted); font-weight:400;">(${p.patient_id || '-'})</span></div>
                                        <div style="font-size:0.75rem; color:var(--text-muted);">${p.age} yrs • ${p.gender}</div>
                                        <div style="font-size:0.78rem; color:var(--accent, #3b82f6); font-weight:600; margin-top:2px;">🕒 Last Visit: ${p.last_visit_formatted || p.created_at || '-'} ${p.doctor_name ? '• ' + p.doctor_name : ''}</div>
                                    </div>
                    ...
                    toast('No patient found with provided details', 'error');
                }
            } catch (err) {
        ...
        let receptionPhoneTimeout;
        window.lastHandledPhone = null;
        window.checkReceptionPhoneInput = function (val) {
            const phone = String(val || '').trim();
            if (window.lastHandledPhone && phone === window.lastHandledPhone) {
                return;
            }
            const banner = $('#receptionLastVisitBanner');
            const bannerText = $('#receptionLastVisitText');
            const quickBox = $('#fetchQuickLastVisit');
            const quickDt = $('#quickLastVisitDateTime');
            const quickMeta = $('#quickLastVisitMeta');

            if (phone.length >= 10) {
                clearTimeout(receptionPhoneTimeout);
                receptionPhoneTimeout = setTimeout(async () => {
                    try {
                        const res = await api('/api/patients/lookup_by_phone?phone=' + safeEncodeURIComponent(phone));
                        window.lastHandledPhone = phone;
                        if (res.success && res.last_visit_formatted) {
                            let docText = res.doctor_name ? ` • Doctor: <strong>${res.doctor_name}</strong>` : '';
                            let tokenText = res.token ? ` • Prev Token: <strong>${res.token}</strong>` : '';
                            let idText = res.patient_id ? ` • ID: <strong>${res.patient_id}</strong>` : '';
                            
                            let moreLink = ` <a href="javascript:void(0)" onclick="showPatientHistory('${phone}', '${res.patient_id || ''}', '${res.name.replace(/'/g, "\\'")}')" style="margin-left: 8px; color: var(--accent, #3b82f6); font-weight: 600; text-decoration: underline;">More</a>`;

                            if (banner && bannerText) {
                                bannerText.innerHTML = `Patient: <strong>${res.name}</strong> • Last visited on: <strong>${res.last_visit_formatted}</strong>${docText}${tokenText}${idText}${moreLink}`;
                                banner.style.display = 'block';
                            }
                            if (quickBox && quickDt) {
                                quickDt.textContent = res.last_visit_formatted;
                                if (quickMeta) quickMeta.innerHTML = `Patient: <strong>${res.name}</strong>${docText}${tokenText}${idText}${moreLink}`;
                                quickBox.style.display = 'block';
                            }
                            if ($('#patName') && !$('#patName').value.trim() && res.name) {
                                $('#patName').value = res.name;
                            }
                        } else {
                            if (banner) banner.style.display = 'none';
                            if (quickBox) quickBox.style.display = 'none';
        ...
        window.showPatientHistory = async function(phone, patient_id, name) {
            try {
                const res = await api('/api/patients/history_all?phone=' + safeEncodeURIComponent(phone) + '&patient_id=' + safeEncodeURIComponent(patient_id) + '&name=' + safeEncodeURIComponent(name));
                if (res.success && res.history && res.history.length > 0) {
                    ...
                    header.innerHTML = '<h3 style="margin:0 0 15px 0; color:var(--text-primary);">Visit History: ' + name + '</h3>';
                    ...
                    res.history.forEach(v => {
                        html += '<tr style="border-bottom: 1px solid #eee;">' +
                            '<td style="padding: 8px;">' + (v.visit_date || '-') + '</td>' +
                            '<td style="padding: 8px;">' + (v.visit_time || '-') + '</td>' +
                            '<td style="padding: 8px;">' + (v.doctor_name || '-') + '</td>' +
                            '<td style="padding: 8px; font-weight:600;">' + (v.token || '-') + '</td>' +
                        '</tr>';
                    });
                    html += '</tbody></table>';
                    ...
                    footer.innerHTML = '<button onclick="this.closest(\'div[style*=\\\'100vw\\\']\').remove()" style="padding: 6px 16px; border: none; background: #e2e8f0; color: #1e293b; border-radius: 4px; cursor: pointer; font-weight: 600;">Close</button>';
        ...
        function applyPatientData(p) {
            ...
            if ($('#patName')) $('#patName').value = p.name || '';
            if ($('#patAge')) $('#patAge').value = p.age || '';
            if ($('#patGender')) $('#patGender').value = p.gender || '';
            ...
            clearTimeout(receptionPhoneTimeout);
            window.lastHandledPhone = phoneVal;
            if ($('#patPhone')) $('#patPhone').value = phoneVal;

            if ($('#patAddress')) $('#patAddress').value = p.address || '';

            // Explicitly clear medical data (vitals & issues) for the new visit
            if ($('#patComplaint')) $('#patComplaint').value = '';
            if ($('#patBP')) $('#patBP').value = '';
            if ($('#patTemp')) $('#patTemp').value = '';
            if ($('#patPulse')) $('#patPulse').value = '';
            if ($('#patWeight')) $('#patWeight').value = '';
            if ($('#patHeight')) $('#patHeight').value = '';
            if ($('#patSpO2')) $('#patSpO2').value = '';

            const lastDt = p.last_visit_formatted || p.created_at || null;

            // Display Quick Fetch Last Visit Box
            const quickBox = $('#fetchQuickLastVisit');
            const quickDt = $('#quickLastVisitDateTime');
            const quickMeta = $('#quickLastVisitMeta');
            if (quickBox && quickDt) {
                if (lastDt) {
                    quickDt.textContent = lastDt;
                    let docText = p.doctor_name ? ` • Doctor: ${p.doctor_name}` : '';
                    let tokenText = p.token ? ` • Prev Token: ${p.token}` : '';
                    let idText = p.patient_id ? ` • ID: ${p.patient_id}` : '';
                    if (quickMeta) quickMeta.innerHTML = `Patient: <strong>${p.name || ''}</strong>${docText}${tokenText}${idText}`;
                    quickBox.style.display = 'block';
                } else {
            ...
            // Display Last Visit Banner in Registration Form
            const banner = $('#receptionLastVisitBanner');
            const bannerText = $('#receptionLastVisitText');
            if (banner && bannerText) {
                if (lastDt) {
                    let docText = p.doctor_name ? ` • Doctor: <strong>${p.doctor_name}</strong>` : '';
                    let tokenText = p.token ? ` • Prev Token: <strong>${p.token}</strong>` : '';
                    let idText = p.patient_id ? ` • ID: <strong>${p.patient_id}</strong>` : '';
                    bannerText.innerHTML = `Patient: <strong>${p.name || ''}</strong> • Last visited on: <strong>${lastDt}</strong>${docText}${tokenText}${idText}`;
                    banner.style.display = 'block';
                } else {
    ...
    function renderDoctorQueue(patients) {
        ...
        container.innerHTML = patients.map(p => {
            ...
            let detailsHtml = '';
            let lastVisitBadge = '';
            if (p.prev_visit_formatted) {
                lastVisitBadge = `<div class="pc-meta" style="color:var(--accent, #3b82f6); font-weight:600; font-size:0.78rem;">🕒 Last Visit: ${p.prev_visit_formatted}${p.prev_doctor_name ? ' (' + p.prev_doctor_name + ')' : ''}</div>`;
            } else {
                lastVisitBadge = `<div class="pc-meta" style="color:var(--text-muted); font-size:0.74rem;">New Patient (First Visit)</div>`;
            }
    ...
    window.openPrescribe = async function (pid) {
        ...
        try {
            ...
            let lastVisitItem = '';
            if (p.last_visit_formatted) {
                lastVisitItem = `
                    <div class="detail-item" style="grid-column: 1 / -1; background: rgba(59, 130, 246, 0.08); border: 1px solid rgba(59, 130, 246, 0.25); border-left: 4px solid var(--accent, #3b82f6); padding: 10px 14px; border-radius: 8px;">
                        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:6px;">
                            <div>
                                <div class="label" style="font-size:0.75rem; font-weight:700; text-transform:uppercase; letter-spacing:0.05em; color:var(--accent, #3b82f6);">Previous Visit Details</div>
                                <div class="value" style="font-size:0.92rem; font-weight:700; margin-top:2px; color:var(--text-primary);">
                                    🕒 ${p.last_visit_formatted} ${p.last_visit_doctor ? ' • Dr: ' + p.last_visit_doctor : ''}
                                </div>
                            </div>
                            ${p.last_visit_token ? `<span class="badge badge-outline" style="font-size:0.75rem; font-weight:600;">Token: ${p.last_visit_token}</span>` : ''}
                        </div>
                    </div>
                `;
            } else {
                lastVisitItem = `
                    <div class="detail-item" style="grid-column: 1 / -1; background: var(--bg-secondary); border: 1px solid var(--border); padding: 8px 14px; border-radius: 8px;">
                        <div class="label" style="font-size:0.75rem; text-transform:uppercase; letter-spacing:0.05em; color:var(--text-muted);">Visit Status</div>
                        <div class="value" style="font-size:0.85rem; color:var(--text-muted); font-weight:500;">First Visit (New Patient)</div>
                    </div>
                `;
            }
    ...
    window.renderPharmacyTable = function(patients) {
        ...
        body.innerHTML = filtered.map(p => {
            ...
            const lastVisitBadge = p.prev_visit_formatted ?
                `<div style="font-size: 0.74rem; color: var(--accent, #3b82f6); font-weight: 600; margin-top: 3px;">🕒 Last Visit: ${p.prev_visit_formatted}</div>` : '';
    ...
    window.openMedicineModal = async function (prescId, name, diag, presc) {
        ...
        // Display Last Visit info in Medicine Modal if returning patient
        const lastVisitContainer = document.getElementById('medModalLastVisitContainer');
        const lastVisitSpan = document.getElementById('medModalLastVisit');
        const patient = window.currentPatients ? window.currentPatients.find(p => String(p.presc_id) === String(prescId)) : null;
        if (lastVisitContainer && lastVisitSpan) {
            if (patient && (patient.prev_visit_formatted || patient.prev_visit_at)) {
                const dt = patient.prev_visit_formatted || patient.prev_visit_at;
                const doc = patient.prev_doctor_name ? ` • Dr: ${patient.prev_doctor_name}` : '';
                lastVisitSpan.textContent = `${dt}${doc}`;
                lastVisitContainer.style.display = 'block';
            } else if (patient && patient.phone) {
                api('/api/patients/lookup_by_phone?phone=' + safeEncodeURIComponent(patient.phone)).then(res => {
                    if (res && res.success && res.last_visit_formatted && res.is_returning) {
                        const doc = res.doctor_name ? ` • Dr: ${res.doctor_name}` : '';
                        lastVisitSpan.textContent = `${res.last_visit_formatted}${doc}`;
                        lastVisitContainer.style.display = 'block';
                    } else {
                        lastVisitSpan.innerHTML = '<span style="color:var(--text-muted); font-weight:normal;">New Patient (First Visit)</span>';
                        lastVisitContainer.style.display = 'block';
                    }
                }).catch(() => {
                    lastVisitContainer.style.display = 'none';
                });
            } else {
                lastVisitContainer.style.display = 'none';
            }
    ...
    function _renderMedItem(item, isFirst, isAddNewBrand, isWithoutBrand) {
        ...
        if (isTablet) {
            ...
            let isNeg = item.stock < 0;
            let absStock = Math.abs(item.stock);
            let strips = Math.floor(absStock / tps), loose = absStock % tps;
            let sign = isNeg ? '-' : '';
            let stockText = sign + strips + ' Strips' + (loose > 0 ? ` + ${loose} Tabs` : '') + stockWarn;
    ...
    // Auto-fill direct customer name by phone and show last visit
    let phoneLookupTimeout;
    window.checkDirectCustomerPhone = function (phone) {
        const lastVisitBox = document.getElementById('directLastVisitContainer');
        const lastVisitText = document.getElementById('directLastVisitText');
        if (phone.length >= 10) {
            ...
            phoneLookupTimeout = setTimeout(async () => {
                try {
                    ...
                    if (res.success && res.name) {
                        ...
                        if (lastVisitBox && lastVisitText && res.last_visit_formatted) {
                            let src = res.last_visit_source || 'Visit';
                            let doc = res.doctor_name ? ` • Dr: ${res.doctor_name}` : '';
                            lastVisitText.innerHTML = `<strong>${res.last_visit_formatted}</strong> (${src}${doc})`;
                            lastVisitBox.style.display = 'block';
                        }
                    } else {
                        if (lastVisitBox) lastVisitBox.style.display = 'none';
                    }
                } catch (e) { console.error(e); }
            }, 500);
        } else {
    ...
    window.openDirectPharmacy = async function () {
        ...
        const directLastVisit = document.getElementById('directLastVisitContainer');
        if (directLastVisit) directLastVisit.style.display = 'none';
    ...
    let _patientsRequestSeq = 0;
    window.loadPatients = async function () {
        const reqSeq = ++_patientsRequestSeq;
        try {
            const patients = await api(`/api/patients?as_role=${PAGE_ROLE}`);
            if (reqSeq !== _patientsRequestSeq) {
                // Ignore stale response if a newer patient request was dispatched
                return;
            }

            if (PAGE_ROLE === 'receptionist') {
                window.currentPatients = patients;
                window.lastPatientsLoadedAt = Date.now();
                window.receptionPatientsDirty = false;
    ...
    function renderReceptionistTable(patients) {
        ...
        if (doctors.length === 0) {
            const docMap = {};
            patients.forEach(p => {
                if (p.doctor_id && !docMap[p.doctor_id]) {
                    docMap[p.doctor_id] = { id: p.doctor_id, doctor_type: p.doctor_type, display_name: p.doctor_name };
                }
            });
            doctors = Object.values(docMap);
        }

        // Single-pass O(N) grouping of patients by doctor_id
        const patientsByDoctor = {};
        for (let i = 0; i < patients.length; i++) {
            const p = patients[i];
            const did = String(p.doctor_id);
            if (!patientsByDoctor[did]) {
                patientsByDoctor[did] = [];
            }
            patientsByDoctor[did].push(p);
        }
        ...
        for (let idx = 0; idx < doctors.length; idx++) {
            ...
            const docPatients = patientsByDoctor[did] || [];
            const docNameFormatted = formatDoctorName(doc.display_name, doc.doctor_type);
                ...
                statsHtml += `
                    <div class="stat-card">
                        ...
                        <div>
                            ...
                            <div class="stat-label">${docNameFormatted} Queue</div>
                        </div>
                    </div>
                `;
            }
            ...
            html += `
                <div class="content-card" style="margin-bottom: 32px;">
                    <div class="card-header">
                        <h2 style="flex: 1;">${docNameFormatted}</h2>
            ...
            if (docPatients.length === 0) {
                ...
                `;
                let rowsHtml = '';
                for (let pi = 0; pi < docPatients.length; pi++) {
                    const p = docPatients[pi];
                    const statusClass = p.status === 'waiting' ? 'badge-waiting'
                        : p.status === 'prescribed' ? 'badge-consulted' : 'badge-completed';
                    const displayStatus = p.status === 'prescribed' ? 'Consulted' : p.status;
                    const lastVisitBadge = p.prev_visit_formatted ?
                        `<div style="font-size:0.75rem; color:var(--accent, #3b82f6); font-weight:600; margin-top:2px;">🕒 Last Visit: ${p.prev_visit_formatted}</div>` : '';
                    rowsHtml += `<tr>
                        <td><strong>${p.token}</strong></td>
